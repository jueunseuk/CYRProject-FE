...
const InventoryUseModal = ({onClose, selectItem}) => {
    ...
    const handleClickInventoryItem = async (item) => {
        try {
            ...
            const response = await UI.useInventoryItem(item.userInventoryId, {otherId, postId});
            switch(response.data.type) {
                ...
                case "GLASS_GIFT_TICKET" : {
                    alert(`아이템을 사용해서 ${response.data.data}님에게 성공적으로 유리 조각을 선물했습니다.`);
                    break;
                }
                case "RANDOM_EMOTICON_BOX" : {
                    alert(`아이템을 사용해서 성공적으로 ${response.data.data} 이모티콘을 획득했습니다.`);
                    break;
                }
                case "LOUD_SPEAKER" : {
                    alert(`아이템을 사용해서 성공적으로 ${response.data.data} 게시글을 메인 페이지에 띄웠습니다.`);
                    break;
                }
                default : {
    ...
    const fetchPostList = async () => {
        try {
            const response = await U.getUserPosts(user.userId, {page:0, sort: "createdAt", size: 100, direction: "DESC"});
    ...
    useEffect(() => {
        if(selectItem.shopItemId === 402) {
            fetchUserList();
        }
        if(selectItem.shopItemId === 405) {
            fetchPostList();
        }
    }, []);
    ...
    return (
        <>
            <S.Wrapper>
                <S.Content>
                    <S.Text $size={"18px"}><S.Text $size={"18px"} $weight={"600"}>{selectItem.name}</S.Text> 아이템을 {targetName ? `"${targetName}"에게 `: ""}사용하시겠습니까?</S.Text>
                    {selectItem.description && <BC.Text $color={"#878787"}>{selectItem.description}</BC.Text>}
                    <BC.Text></BC.Text>
                    <S.HorizontalWrapper>
                ...
                {selectItem.shopItemId === 405 &&
                    <BC.VerticalWrapper $gap={"3px"} 
                        style={{backgroundColor: "white", width: "300px", maxHeight: "500px", boxShadow: "0px 0px 16px rgb(0, 0, 0, 0.5)", padding: "15px 25px", borderRadius: "10px", overflowY: "auto", cursor: "pointer"}}
                    >
                        <BC.Text $size={"15px"} $weight={"600"}>작성 게시글 목록</BC.Text>
                        {data?.map((post) => (
                            <BC.HorizontalWrapper key={post.postId} $jc={"flex-start"} $gap={"10px"} style={{width: "100%", backgroundColor: targetId === post.postId ? "#eee" : "", padding: "5px", borderRadius: "8px", transition: "all 0.2s ease"}}
                                onClick={() => {setTargetId(post.postId); setTargetName(post.title);}}
                            >
                                <BC.Text $size={"13px"} $weight={"600"}>[{post.boardKorean}]</BC.Text>
                                <BC.Text $size={"13px"} style={{width: "50%", textOverflow: "ellipsis", overflowX: "hidden", whiteSpace: "nowrap"}}>{post.title}</BC.Text>
                            </BC.HorizontalWrapper>
                        ))}
                    </BC.VerticalWrapper>
                }
