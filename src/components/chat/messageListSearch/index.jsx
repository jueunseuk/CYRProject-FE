...
import join from "@/assets/icon/chat/join.svg";
...
const MessageListSearch = () => {
    ...
    const fetchMessageList = async () => {
        try {
            const response = await C.getOtherChatRoomList();
    ...
    const handleJoinChatRoom = async (chatRoom) => {
        try {
            const response = await C.joinChatRoom(chatRoom.chatRoomId);

            const joinChatRoom = {
                "chatRoomId": chatRoom.chatRoomId,
                "name": chatRoom.name,
                "totalMember": chatRoom.totalMember+1
            };
            
            if(response.data) {
                setSelectChatRoom(joinChatRoom);
                ...
                :
                    filteredData.map((chatRoom) => (
                        <S.ChatListItem key={chatRoom.chatRoomId} onClick={() => {setSelectChatRoom(chatRoom); setOpenChatPageModel(true);}}>
                            <BC.Text $size={"16px"} $weight={"600"} style={{width: "60%"}}>
                                {chatRoom.name}
                            ...
                            <BC.Text $color={"#878787"} style={{minWidth: "35%"}}>마지막 대화 : {formatDate(chatRoom.lastUpdatedAt, 3)}</BC.Text>
                            <S.ChatPlusIcon src={join} title="그룹 채팅에 참여하기" onClick={() => handleJoinChatRoom(chatRoom)}/>
                        </S.ChatListItem>
                ))}
            </BC.VerticalWrapper>
        </S.Wrapper>
