...
const CommentSearch = ({data, pagination}) => {
                ...
                <S.PostItem key={post.postId} onClick={() => handleNavigateUserPage(post)}>
                    <BC.VerticalWrapper $ai={"flex-start"} $gap={"5px"} style={{width: "600px"}}>
                        <BC.HorizontalWrapper $gap={"8px"}>
                            <BC.Text $size={"15px"} $weight={"600"} dangerouslySetInnerHTML={{ __html: post.content }}></BC.Text>
                            <BC.Text $size={"13px"} $color={"#878787"}>{formatDate(post.createdAt, 1)}</BC.Text>
                        </BC.HorizontalWrapper>
                        <BC.HorizontalWrapper $gap={"3px"}>
                            <BC.Text $size={"13px"} $color={"#878787"}>[ {post.korean} ]</BC.Text>
                            <BC.Text $size={"13px"} $color={"#878787"} dangerouslySetInnerHTML={{ __html: post.title }}/>
                        </BC.HorizontalWrapper>
                    </BC.VerticalWrapper>
                </S.PostItem>
            ))}
