...
const UserPosts = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await U.getUserPosts(userId, {page: page > 0 ? page - 1 : 0, sort, size, direction});
    ...
    return (
        <>
            <S.Wrapper>
                <S.Title>{userNickname}님이 작성한 글</S.Title>
                <S.Description>{userNickname}님이 지금까지 작성했던 글을 한 눈에 모아서 볼 수 있어요.</S.Description>
                <S.TableHeader>
                    <S.TextArea>
