...
const UserEmpathizedPosts = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await U.getUserEmpathyPosts(userId, {page: page > 0 ? page - 1 : 0, sort, size, direction});
    ...
    return (
        <>
            <S.Wrapper>
                <S.Title>{userNickname}님이 공감한 글</S.Title>
                <S.Description>{userNickname}님이 지금까지 공감을 눌렀던 글을 한 눈에 모아서 볼 수 있어요.</S.Description>
                <S.TableHeader>
                    <S.TextArea>
...
export default UserEmpathizedPosts;