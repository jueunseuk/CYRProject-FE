...
const UserImages = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const response = await U.getUserImages(userId, {page: page > 0 ? page - 1 : 0, sort});
    ...
    return (
        <>
            <S.Wrapper>
                <S.Title>{userNickname}님이 올린 사진</S.Title>
                <S.Description>{userNickname}님이 지금까지 갤러리에 올린 유리의 사진들을 모아서 볼 수 있어요.</S.Description>
                <S.Header>
                    <S.TextArea>
